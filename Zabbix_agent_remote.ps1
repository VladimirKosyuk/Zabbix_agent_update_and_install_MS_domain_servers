...
Host need to be enabled for access via invoke-command;
Host account in AD need to enable delegation for any service via Kerberos auth;
...
#vars
$Output = '' # type smb path to log between ''
...
$Auth_login = "" #type login between ""
$Auth_pass = "" #type password between ""

#auth
$Cred = New-Object System.Management.Automation.PSCredential($Auth_login,$($Auth_pass | ConvertTo-SecureString -AsPlainText -Force))
...
#do script for each of in matrix
foreach ($pc in $list) {
$Archangel_agent = Invoke-Command -ComputerName $pc -ScriptBlock {
#main begin
...
Wait-Job $Archangel_agent -Timeout 300
if ($Archangel_agent.State -eq 'Completed') {
$Archangel_agent |select State, Location, PSBeginTime, PSEndTime| Out-File $Output\$Domain_name"_"$Debag_Date"_"Progress_agent.log -Append
} 
    else {
        $Archangel_agent |select State, Location, PSBeginTime, PSEndTime| Out-File $Output\$Domain_name"_"$Debag_Date"_"Progress_agent.log -Append
        Stop-Job -Id $Archangel_agent.Id
} 
Receive-Job $Archangel_agent
